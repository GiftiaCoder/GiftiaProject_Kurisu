...
__declspec(dllimport) void calculate_layer_grad(real output[], real target[], real grad[], real study_rate, size_t output_num);
...
__declspec(dllimport) void translate_data_format(void *dst, const void *src, size_t count, enum_translate_type type);

__declspec(dllimport) void set_value(real data[], size_t count, real val);
__declspec(dllimport) void set_rand_value(real data[], size_t count);
